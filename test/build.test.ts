...
const CD_SIGNERS: CdSignerEntry[] = [
    { role: "cd-signer", kind: "production", subjectKeyId: "dddddd", derHex: "30dd" },
];
...
let tmpDir: string;
beforeEach(async () => { tmpDir = await mkdtemp(join(tmpdir(), "dcl-build-")); });
afterEach(async () => { await rm(tmpDir, { recursive: true }); });

describe("buildPackage", () => {
    it("paa-roots.jsonl sorted by subjectKeyId ascending", async () => {
        await buildPackage({ paaRoots: PAA_ROOTS, cdSigners: CD_SIGNERS, vendors: VENDORS, outDir: tmpDir, builtAt: "2026-05-14T02:00:00.000Z", sources: SOURCES });
    ...
    it("vendors.json sorted by vendorId ascending", async () => {
        await buildPackage({ paaRoots: PAA_ROOTS, cdSigners: CD_SIGNERS, vendors: VENDORS, outDir: tmpDir, builtAt: "2026-05-14T02:00:00.000Z", sources: SOURCES });
        const arr = JSON.parse(await readFile(join(tmpDir, "data", "vendors.json"), "utf8")) as Array<{ vendorId: number }>;
    ...
    it("same input → byte-identical JSONL (deterministic)", async () => {
        const opts = { paaRoots: PAA_ROOTS, cdSigners: CD_SIGNERS, vendors: VENDORS, outDir: tmpDir, builtAt: "2026-05-14T02:00:00.000Z", sources: SOURCES };
